    ...
    const do_validate = function() {
        frappe.call({
            ...
            callback(r) {
                ...
                const summary = r.message;
                const errorItems = (summary.errors || []).slice(0, 15).map((row) => `<li>${frappe.utils.escape_html(row)}</li>`).join('');
                const warningItems = (summary.warnings || []).slice(0, 15).map((row) => `<li>${frappe.utils.escape_html(row)}</li>`).join('');
                const costCenterItems = (summary.would_create_cost_centers || []).slice(0, 15).map((row) => `<li>${frappe.utils.escape_html(row)}</li>`).join('');
