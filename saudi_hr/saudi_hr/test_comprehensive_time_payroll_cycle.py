...
class TestComprehensiveTimePayrollCycle(FrappeTestCase):
	...
	def test_leave_entitlements_pay_and_overlap_cycle(self):
		...
		invalid_annual = frappe.get_doc(
			{
				"doctype": "Saudi Annual Leave",
				"employee": self.employee,
				"company": self.company,
				"leave_start_date": "2026-09-05",
				"leave_end_date": "2026-09-04",
			}
		)
		with self.assertRaises(frappe.ValidationError):
		...
		overlapping_sick = frappe.get_doc(
			{
				"doctype": "Saudi Sick Leave",
				"employee": self.employee,
				"company": self.company,
		...
		disbursement = frappe.get_doc(
			{
				...
				"company": self.company,
				"leave_year": 2026,
				"leave_from_date": "2026-08-02",
				"leave_to_date": "2026-08-06",
				"leave_days_to_pay": 5,
				"disbursement_type": select_option("Annual Leave Disbursement", "disbursement_type", "Full Salary"),
			}
		).insert(ignore_permissions=True)
		...
		special = frappe.get_doc(
			{
				...
				"employee": self.employee,
				"company": self.company,
				"leave_type": select_option("Special Leave", "leave_type", "Marriage"),
				"leave_start_date": "2026-11-01",
				"leave_end_date": "2026-11-05",
				"documentation_attached": 1,
			}
		).insert(ignore_permissions=True)
		...
		parental = frappe.get_doc(
			{
				"doctype": "Maternity Paternity Leave",
				"employee": self.employee,
				"company": self.company,
				"leave_type": select_option("Maternity Paternity Leave", "leave_type", "Paternity"),
				"leave_start_date": "2026-12-01",
				"medical_certificate_attached": 1,
			}
		).insert(ignore_permissions=True)
	...
	def test_overtime_flexible_and_working_limits_cycle(self):
		overtime = frappe.get_doc(
			{
				...
				"normal_hours": 8,
				"overtime_hours": 4,
				"compensation_method": select_option("Overtime Request", "compensation_method", "Compensatory Leave"),
				"written_consent_reference": "CONSENT-QA-001",
				"approval_status": select_option("Overtime Request", "approval_status", "Approved"),
			}
		).insert(ignore_permissions=True)
		...
		for values in (
			{"normal_hours": 8, "overtime_hours": 5, "written_consent_reference": "CONSENT"},
			{"normal_hours": 8, "overtime_hours": 2, "written_consent_reference": ""},
		):
		...
		flexible = frappe.get_doc(
			{
				"doctype": "Work Arrangement Control",
				"employee": self.employee,
				"company": self.company,
				"arrangement_type": select_option("Work Arrangement Control", "arrangement_type", "Flexible Work"),
				"status": select_option("Work Arrangement Control", "status", "Active"),
				"start_date": "2026-01-01",
				"end_date": "2026-12-31",
				"monthly_hours": 120,
				"platform_reference": "QIWA-QA-001",
			}
		).insert(ignore_permissions=True)
		...
		temporary = frappe.get_doc(
			{
				"doctype": "Work Arrangement Control",
				"employee": self.employee,
				"company": self.company,
				"arrangement_type": select_option("Work Arrangement Control", "arrangement_type", "Temporary Work"),
				"status": select_option("Work Arrangement Control", "status", "Active"),
				"start_date": "2026-01-01",
				"end_date": "2026-04-15",
			}
		...
		working_time = frappe.get_doc(
			{
				"doctype": "Working Time Compliance Check",
				"employee": self.employee,
				"company": self.company,
				"check_date": "2026-08-17",
				"actual_daily_hours": 9,
				"actual_weekly_hours": 52,
			}
		).insert(ignore_permissions=True)
