...
class TestComprehensiveGovernanceCycle(FrappeTestCase):
	def setUp(self):
		...
		self.department = frappe.get_doc(
			{"doctype": "Department", "department_name": department_label, "company": self.company, "is_group": 0}
		).insert(ignore_permissions=True).name
		frappe.db.set_value(
			"Employee",
			self.employee,
			{"department": self.department, "date_of_joining": "2023-01-01"},
		)
	...
	def test_policy_acknowledgement_training_and_regulatory_cycle(self):
		policy = frappe.get_doc(
			{
				...
				"policy_title": "سياسة السلوك المهني وحماية الحقوق",
				...
				"policy_version": "1.0-QA",
				"target_scope": select_option("HR Policy Document", "target_scope", "Department Employees"),
				"target_department": self.department,
				"acknowledgement_required": 1,
				"acknowledgement_due_days": 5,
				"compliance_risk": select_option("HR Policy Document", "compliance_risk", "High"),
				"summary": "سياسة ثنائية اللغة توضح السلوك والحقوق وقنوات الاعتراض.",
			}
		...
		ack_name = frappe.db.get_value(
			...
			{"policy_document": policy.name, "employee": self.employee},
			"name",
		)
		...
		training = frappe.get_doc(
			{
				...
				"company": self.company,
				"training_title": "السلامة المهنية والامتثال لنظام العمل",
				"training_type": select_option("Training Record", "training_type", "Mandatory Safety"),
				"training_start_date": today(),
				"training_end_date": today(),
				"attendance_status": select_option("Training Record", "attendance_status", "Attended"),
				"result": select_option("Training Record", "result", "Pass"),
				"training_provider_rating": select_option("Training Record", "training_provider_rating", "5 - Excellent"),
			}
		).insert(ignore_permissions=True)
		...
		task = frappe.get_doc(
			{
				...
				"task_title": "مراجعة تطبيق سياسة السلوك",
				"company": self.company,
				"task_date": today(),
				"due_date": add_days(today(), 7),
				"assigned_to": "Administrator",
				"policy_document": policy.name,
				"priority": select_option("Saudi Regulatory Task", "priority", "High"),
				"task_category": select_option("Saudi Regulatory Task", "task_category", "Policy"),
	...
	def test_grievance_investigation_discipline_appeal_and_dispute_cycle(self):
		grievance = frappe.get_doc(
			{
				...
				"employee": self.employee,
				"company": self.company,
				"grievance_date": "2026-07-01",
				"grievance_type": select_option("Employee Grievance", "grievance_type", "Manager Conduct"),
				"grievance_channel": select_option("Employee Grievance", "grievance_channel", "Portal"),
				"severity": select_option("Employee Grievance", "severity", "High"),
				"assigned_to": "Administrator",
				"grievance_summary": "طلب مراجعة إجراء إداري وسماع إفادة الموظف.",
			}
		).insert(ignore_permissions=True)
		...
		investigation = frappe.get_doc(
			{
				...
				"company": self.company,
				"case_type": select_option("Investigation Record", "case_type", "Misconduct"),
				"allegation_date": "2026-07-01",
				"investigator": "Administrator",
				"investigation_start_date": "2026-07-02",
				"investigation_end_date": "2026-07-03",
				"allegation_summary": "مخالفة إجرائية تستلزم تحقيقًا موثقًا.",
				"findings": "ثبتت المخالفة البسيطة بعد سماع الإفادات.",
				"recommendation": "إنذار كتابي أول وتدريب تصحيحي.",
			}
		).insert(ignore_permissions=True)
		...
		procedure = frappe.get_doc(
			{
				...
				"investigation_record": investigation.name,
				"violation_type": select_option("Disciplinary Procedure", "violation_type", "Misconduct"),
				"incident_description": "إجراء تأديبي متدرج قائم على التحقيق.",
				"penalty_type": select_option("Disciplinary Procedure", "penalty_type", "First Written Warning"),
				"penalty_start_date": "2026-07-04",
				"decision_notes": "اعتماد إنذار أول مع حق الاستئناف.",
				"hr_manager_approval": 1,
				"appeal_date": "2026-07-11",
			}
		).insert(ignore_permissions=True)
		...
		appeal = frappe.get_doc(
			{
				...
				"disciplinary_procedure": procedure.name,
				"employee": self.employee,
				"company": self.company,
				"appeal_date": "2026-07-05",
				"appeal_type": select_option("Disciplinary Appeal", "appeal_type", "Against Warning"),
				"assigned_to": "Administrator",
				"hearing_date": "2026-07-06",
				"grounds_for_appeal": "طلب مراجعة تناسب الجزاء مع الواقعة.",
				"decision": select_option("Disciplinary Appeal", "decision", "Modified"),
		...
		dispute = frappe.get_doc(
			{
				"doctype": "Labor Dispute",
				"employee": self.employee,
				"company": self.company,
				"dispute_date": "2026-07-08",
				"dispute_type": select_option("Labor Dispute", "dispute_type", "Overtime"),
				"dispute_description": "نزاع تجريبي لاختبار مسار الوساطة الداخلية.",
			}
		).insert(ignore_permissions=True)
	...
	def test_injury_medical_inspection_and_corrective_action_cycle(self):
		injury = frappe.get_doc(
			{
				...
				"employee": self.employee,
				"company": self.company,
				"injury_date": add_days(today(), -2),
				"injury_type": select_option("Work Injury", "injury_type", "Industrial Accident"),
				"severity": select_option("Work Injury", "severity", "Minor"),
				"injury_description": "إصابة بسيطة أثناء مناولة مواد مكتبية.",
				"medical_treatment_type": select_option("Work Injury", "medical_treatment_type", "First Aid"),
				"gosi_form_25_submitted": 1,
				"gosi_case_number": f"GOSI-QA-{frappe.generate_hash(length=8)}",
			}
		).insert(ignore_permissions=True)
		...
		medical = frappe.get_doc(
			{
				...
				"employee": self.employee,
				"company": self.company,
				"examination_type": select_option("Medical Examination", "examination_type", "Post-Injury"),
				"examination_date": today(),
				"work_injury_reference": injury.name,
				"fitness_result": select_option("Medical Examination", "fitness_result", "Fit with Restrictions"),
				"restrictions_notes": "تجنب حمل الأوزان لمدة يومين.",
			}
		).insert(ignore_permissions=True)
		...
		invalid_medical = frappe.get_doc(
			{
				"doctype": "Medical Examination",
				"employee": self.employee,
				"company": self.company,
				"examination_type": select_option("Medical Examination", "examination_type", "Periodic"),
				"examination_date": today(),
				"fitness_result": select_option("Medical Examination", "fitness_result", "Temporarily Unfit"),
			}
		)
		with self.assertRaises(frappe.ValidationError):
		...
		inspection = frappe.get_doc(
			{
				"doctype": "Labor Inspection",
				"inspection_title": "تفتيش امتثال تجريبي شامل",
				"inspection_authority": select_option("Labor Inspection", "inspection_authority", "Internal Audit"),
				"company": self.company,
				"inspection_date": today(),
				"inspection_scope": select_option("Labor Inspection", "inspection_scope", "Routine Inspection"),
				"internal_owner": "Administrator",
				"follow_up_due_date": add_days(today(), 7),
				"findings_summary": "ملاحظة واحدة تتطلب إجراءً تصحيحيًا موثقًا.",
				"violations": [
					{
						"violation_category": select_option("Labor Inspection Violation", "violation_category", "Record Keeping"),
						"severity": select_option("Labor Inspection Violation", "severity", "High"),
						"status": select_option("Labor Inspection Violation", "status", "Open"),
						"violation_description": "سجل واحد بحاجة إلى استكمال دليل الحفظ.",
						"corrective_action": "إرفاق الدليل وإغلاق الملاحظة.",
						"fine_amount": 5000,
						"correction_due_date": add_days(today(), 7),
					}
				],
			}
		).insert(ignore_permissions=True)
		...
		invalid_inspection = frappe.get_doc(
			{
				"doctype": "Labor Inspection",
				"inspection_title": "تفتيش بلا مخالفات",
				"inspection_authority": select_option("Labor Inspection", "inspection_authority", "Internal Audit"),
				"company": self.company,
				"inspection_date": today(),
				"inspection_scope": select_option("Labor Inspection", "inspection_scope", "Routine Inspection"),
				"findings_summary": "يجب رفض النموذج لأنه بلا صفوف.",
			}
		)
		with self.assertRaises(frappe.ValidationError):
